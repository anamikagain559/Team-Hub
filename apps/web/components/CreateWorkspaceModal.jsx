...
import { X, Loader2, Layout, Palette, Sparkles, Command, ShieldCheck } from 'lucide-react';
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      Swal.fire({
        ...
        text: `"${workspace.name}" has been established.`,
        icon: 'success',
        background: '#0a0a0a',
        color: '#fff',
      ...
      <div className="relative w-full max-w-lg overflow-hidden rounded-[2.5rem] border border-white/10 bg-[#0a0a0a] shadow-2xl animate-in zoom-in-95 duration-300">
        ...
        <div className="relative flex items-center justify-between border-b border-white/5 p-8 bg-white/[0.02]">
          <div className="flex items-center space-x-4">
             ...
             <div>
                <h2 className="text-xl font-black tracking-tight text-white uppercase leading-none">New Hub Deployment</h2>
                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-white/30 mt-1">Establishing Base Protocol</p>
             </div>
          </div>
          <button 
            onClick={onClose} 
            className="rounded-xl p-3 text-white/30 hover:bg-white/5 hover:text-white transition-all"
          >
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="relative p-10 space-y-10">
          <div className="space-y-8">
            <div className="group">
              <label className="flex items-center text-[10px] font-black text-white/40 uppercase tracking-[0.3em] mb-4">
              ...
              <input
                {...register('name')}
                autoFocus
                placeholder="e.g. Apollo Missions, Tactical Ops"
            ...
            <div>
              <label className="flex items-center text-[10px] font-black text-white/40 uppercase tracking-[0.3em] mb-4">
                Briefing / Mission Statement
              </label>
              <textarea
                {...register('description')}
                placeholder="Define the primary objectives for this workspace..."
                rows={3}
